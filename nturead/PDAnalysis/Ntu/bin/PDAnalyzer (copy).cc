...
//  }
  ...
  vector<array<int, 5>> goodBus;
  vector<array<int, 4>> goodPsi2Ss;
  vector<array<int, 2>> goodJPsis;
  ...
  for (size_t iSvt = 0; iSvt < tWriter->svtPt->size(); iSvt++) {
    ...
    for (int i = 0; i < 5; i++) {
      auto iTrkGen = GetClosestTrackGen(trkEta->at(svtTracks[i]), trkPhi->at(svtTracks[i]));
      if (iTrkGen >= 0) {
        tWriter->trkGenId->at(svtTracks[i]) = abs(genId->at(iTrkGen));
      }
      svtTracks[i] = iTrkGen;
    }
...
int PDAnalyzer::GetClosestTrackGen( float eta, float phi) 
{
    double drb = 0.12;
    int best = -1;
...
std::optional<std::array<int, 5>> PDAnalyzer::getBuVertexComponents(int iBuGen) {
  std::array<int, 5> ret = {{-1, -1, -1, -1, -1}};
...
std::optional<std::array<int, 4>> PDAnalyzer::getPsi2SVertexComponents(int iPsi2SGen) {
  std::array<int, 4> ret = {{-1, -1, -1, -1}};
...
std::optional<std::array<int, 2>> PDAnalyzer::getJPsiVertexComponents(int iJPsiGen) {
