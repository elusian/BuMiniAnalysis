...
//    if ( !( event_file % 10000 ) || !( event_tot % 10000 ) )
  ...
  for (int iSvt = 0; iSvt < nSVertices; iSvt++) {
    ...
    tWriter->svtGenMatchRadius->push_back(-1);
    tWriter->svtGenMatchDpt->push_back(-1);
    tWriter->svtGenMatchIndex->push_back(-1);
...
//  }
  ...
  vector<int> goodBuIndex;
  vector<int> goodPsi2SIndex;
  vector<int> goodJPsiIndex;
  ...
  for (int iGen = 0; iGen < nGenP; iGen++) {
    ...
    if (id == 521) {
      ...
      if (decayTracks) {
        ...
        goodBuIndex.push_back(tWriter->genId->size() - 1);
      }
    }
    else if (id == 100443) {
      ...
      if (decayTracks) {
        ...
        goodPsi2SIndex.push_back(tWriter->genId->size() - 1);
      }
    }
    else if (id == 443) {
      ...
      if (decayTracks) {
        ...
        goodJPsiIndex.push_back(tWriter->genId->size() - 1);
      }
    }
  }
...
//  };
  auto drFn = [this](int trk, int gen) -> float {
    return deltaR(this->trkEta->at(trk), this->trkPhi->at(trk), this->genEta->at(gen), this->genPhi->at(gen));
  };
  auto dptFn = [this](int trk, int gen) -> float {
    return abs(this->genPt->at(gen) - this->trkPt->at(trk))/this->genPt->at(gen);
  };
  auto crossDrComp = [this, &drFn, &dptFn](const int* v1, const int* v2, float radius = 0.5, float ptDiff = 1) -> tuple<bool, float, float> {
    int vAssoc2[2] = {v2[0], v2[1]};
    
    if (this->trkCharge->at(v1[0]) != this->genCharge->at(vAssoc2[0])) {
      vAssoc2[0] = v2[1];
      vAssoc2[1] = v2[0];
    }
    
    auto dr1 = drFn(v1[0], vAssoc2[0]);
    auto dr2 = drFn(v1[1], vAssoc2[1]);
    
    auto dpt1 = dptFn(v1[0], vAssoc2[0]);
    auto dpt2 = dptFn(v1[1], vAssoc2[1]);
    
    return {(dr1 < radius and dr2 < radius and dpt1 < ptDiff and dpt2 < ptDiff), max(dr1, dr2), max(dpt1, dpt2)};
  };
...
//    }
    ...
    for (size_t iGoodJPsi = 0; iGoodJPsi < goodJPsis.size(); iGoodJPsi++) {
      auto [comp, dr, dpt] = crossDrComp(svtTracks.data(), goodJPsis[iGoodJPsi].data());
      if (comp) {
        isJPsiGood = 3;
        if (goodJPsis[iGoodJPsi][2] == 0) {
          isJPsiGood = 4;
        }
        tWriter->svtGenMatchRadius->at(iJPsiSV) = dr;
        tWriter->svtGenMatchDpt->at(iJPsiSV) = dpt;
        tWriter->svtGenMatchIndex->at(iJPsiSV) = goodJPsiIndex[iGoodJPsi];
      }
    ...
    if (isJPsiGood >= 3) {
      for (size_t iGoodPsi2S = 0; iGoodPsi2S < goodPsi2Ss.size(); iGoodPsi2S++) {
        auto [comp1, dr1, dpt1] = crossDrComp(svtTracks.data(), goodPsi2Ss[iGoodPsi2S].data());
        auto [comp2, dr2, dpt2] = crossDrComp(svtTracks.data() + 2, goodPsi2Ss[iGoodPsi2S].data() + 2);
        if (comp1 and comp2) {
          
          isPsi2SGood = 3;
          if (goodJPsis[iGoodPsi2S][4] == 0) {
            isPsi2SGood = 4;
          }
          tWriter->svtGenMatchRadius->at(iPsi2SSV) = dr2;
          tWriter->svtGenMatchDpt->at(iPsi2SSV) = dpt2;
          tWriter->svtGenMatchIndex->at(iPsi2SSV) = goodPsi2SIndex[iGoodPsi2S];
        }
      }
    }
    ...
    if (isPsi2SGood >= 3) {
      for (size_t iGoodBu = 0; iGoodBu < goodBus.size(); iGoodBu++) {
        auto [comp1, dr1, dpt1] = crossDrComp(svtTracks.data(), goodBus[iGoodBu].data());
        auto [comp2, dr2, dpt2] = crossDrComp(svtTracks.data() + 2, goodBus[iGoodBu].data() + 2);
        auto dr3 = drFn(svtTracks[4], goodBus[iGoodBu][4]);
        auto dpt3 = dptFn(svtTracks[4], goodBus[iGoodBu][4]);
        auto comp3 = dr3 < 0.5 and dpt3 < 1. and trkCharge->at(svtTracks[4]) == genCharge->at(goodBus[iGoodBu][4]);
        if (comp1 and comp2 and comp3)
        {
          ...
          tWriter->svtGenMatchRadius->at(iSvt) = dr3;
          tWriter->svtGenMatchDpt->at(iSvt) = dpt3;
          tWriter->svtGenMatchIndex->at(iSvt) = goodBuIndex[iGoodBu];
