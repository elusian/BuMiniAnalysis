...
PDAnalyzerUtil::PDAnalyzerUtil() {
  ...
  lastSubSI .run = lastSubSI .event = 0;
...
const std::vector<int>& PDAnalyzerUtil::tipFromSV( int iSV ) {
  if ( chkNewEvent( lastTipSV ) ) {
    tipMap.clear();
//    int iTkIPs;
//    for ( iTkIPs = 0; iTkIPs < nTkIPs; ++iTkIPs ) {
    int iVtxPs;
    for ( iVtxPs = 0; iVtxPs < nVtxPs; ++iVtxPs ) {
      int iTkIPs = tvpTip->at( iVtxPs );
      int jSV = tipSVtx->at( iTkIPs );
      std::vector<int>& tips = tipMap[jSV];
      if ( !tips.size() ) tips.reserve( 10 );
      tips.push_back( iTkIPs );
    }
    updateEvent( lastTipSV );
  }
  return tipMap[iSV];
}
...
const std::vector<int>& PDAnalyzerUtil::subVtxToSV( int iSV ) {
  if ( chkNewEvent( lastSubSI ) ) {
    subInv.clear();
    int iCompVts;
    for ( iCompVts = 0; iCompVts < nCompVts; ++iCompVts ) {
      std::vector<int>& vtxmot = subInv[subPart->at( iCompVts )];
      if ( !vtxmot.size() ) vtxmot.reserve( 4 );
      vtxmot.push_back( subSVtx->at( iCompVts ) );
    }
    updateEvent( lastSubSI );
  }
  return subInv[iSV];
}
